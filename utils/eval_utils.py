...
def eval_refcoco(task, generator, models, sample, **kwargs):
    ...
    def _calculate_ap_score(hyps, refs, thresh=0.5):
        interacts = torch.cat(
            [torch.where(hyps[:, :2] < refs[:, :2], refs[:, :2], hyps[:, :2]), torch.where(hyps[:, 2:] < refs[:, 2:], hyps[:, 2:], refs[:, 2:])],
            dim=1,
        )
    ...
    def _calculate_score(hyps, hyps_det, refs, sample, n_poly_pred, n_poly_gt, vis=True, vis_dir=None):
        ...
        for i in range(b):
            hyps_i = hyps[i]  # format: [array([poly_x1,poly_y1,poly_x2,poly_y2,...,poly_xn,poly_yn])]
            gt_poly = refs[
                i
            ]  # format: tensor([[box_x1,box_y1],[box_x2,box_y2],[poly_x1,poly_y1],[poly_x2,poly_y2],...,[poly_xn,poly_yn],[0,0],[1,1],...,[1,1]])

            # to numpy and flatten
            if "rec" in sample["task"]:
                gt_poly = [gt_poly[:2].cpu().numpy().flatten() * 512]
            else:
                # reformat gt_poly to match the format of hyps_i, remove box coordinates and padding [0,0],[1,1],...,[1,1]
                zero_padding_idx = (gt_poly[:, 0] == 0).nonzero()[0][0].item()
                gt_poly = [gt_poly[2:zero_padding_idx].cpu().numpy().flatten() * 512]

            # pred_mask = get_mask_from_codes(hyps_i, gt_mask.shape[0:2])
            pred_mask = get_mask_from_codes(hyps_i, (512, 512))
            gt_mask = get_mask_from_codes(gt_poly, (512, 512))
            ...
            if vis:
                ...
                # gt_overlayed_fn = f"{uniq_id}_gt_overlayed.jpg"
                # pred_overlayed_fn = f"{uniq_id}_pred_overlayed.jpg"
                ...
                # horizontal concat two images into one array
                overlayed = np.concatenate((pred_overlayed, gt_overlayed), axis=1)
                Image.fromarray(overlayed.astype(np.uint8)).save(os.path.join(vis_dir, f"{uniq_id}_overlayed.jpg"), format="JPEG", quality=80)
                # pred_overlayed = Image.fromarray(pred_overlayed.astype(np.uint8))
                # gt_overlayed = Image.fromarray(gt_overlayed.astype(np.uint8))

                # pred_overlayed.save(os.path.join(vis_dir, pred_overlayed_fn),format='JPEG',quality=80)
    ...
    # support for REC
    if "rec" in sample["task"]:
        gen_out = task.inference_step_valid(models, sample).cpu()
    else:
    ...
    for i in range(b):
        ...
        if "rec" in sample["task"]:
            polygons_pred = np.append([2], gen_out_i[:4])
        else:
            polygons_pred = gen_out_i[4:]
        polygons_pred = np.append(polygons_pred, [2])
        size = len(polygons_pred)
        idx_list = [idx for idx, val in enumerate(polygons_pred) if val == 2]  # 2 indicates separator token
    ...
    # gt = sample['label']
    gt = sample["target"]
    results = [{"uniq_id": sample_id} for i, sample_id in enumerate(sample["id"].tolist())]

    iou_scores, f_scores, ap_scores, cum_I, cum_U = _calculate_score(
        hyps, hyps_det, gt, sample, n_poly_pred=None, n_poly_gt=None, vis=kwargs["vis"], vis_dir=kwargs["vis_dir"]
    )
...
def merge_results(
    ...
    if task.cfg._name == "image_gen":
        ...
        if score_cnt.item() > 0:
            logger.info("score_sum: {}, score_cnt: {}, score: {}".format(score_sum, score_cnt, round(score_sum.item() / score_cnt.item(), 4)))
    else:
        gather_results = None
        if cfg.distributed_training.distributed_world_size > 1:
        ...
        if score_cnt.item() > 0:
            ...
            txt = "sample_cnt: {}, mIoU score: {}, oIoU score: {}, ap det score: {}, f score: {}, J&F: {}\n".format(
                score_cnt,
                round(score_sum.item() / score_cnt.item(), 4),
            ...
            prec_txt = " ".join(
                [f"prec@{prec}: {round(prec_score.item() / score_cnt.item(), 4)}\n" for prec, prec_score in zip(prec_list, prec_score_sum)]
            )
            txt += prec_txt
