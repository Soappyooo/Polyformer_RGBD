...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.checkpoint.write_checkpoints_asynchronously:
        try:
            ...
            logging.exception("Asynchronous checkpoint writing is specified but iopath is " "not installed: `pip install iopath`")
            return
    ...
    freeze_layers(model, ["encoder.embed_images", "encoder.bert"])
    ...
    if cfg.dataset.combine_valid_subsets:
        task.load_dataset("val", combine=True, epoch=1)
    else:
...
def train(cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr) -> Tuple[List[Optional[float]], bool]:
    ...
    progress = progress_bar.progress_bar(
        ...
        tensorboard_logdir=(cfg.common.tensorboard_logdir if distributed_utils.is_master(cfg.distributed_training) else None),
        ...
        azureml_logging=(cfg.common.azureml_logging if distributed_utils.is_master(cfg.distributed_training) else False),
    )
    ...
    #     print(cmd)
    #     os.system(cmd)
    if task.cfg._name == "refcoco" and epoch_itr.epoch % 20 == 0:
        cmd = f"cp {cfg.checkpoint.save_dir}/checkpoint_last.pt {cfg.checkpoint.save_dir}/checkpoint_epoch_{epoch_itr.epoch}.pt"
        print(cmd)
        os.system(cmd)
...
def validate_and_save(
    ...
    do_validate = (
        (
            ...
            or should_stop
            or (cfg.dataset.validate_interval_updates > 0 and num_updates > 0 and num_updates % cfg.dataset.validate_interval_updates == 0)
        )
...
def validate(
    ...
    for subset in subsets:
        ...
        # Initialize data iterator
        itr = trainer.get_valid_iterator(subset).next_epoch_itr(shuffle=False, set_dataset_epoch=False)  # use a fixed valid set
        if cfg.common.tpu:
            itr = utils.tpu_data_loader(itr)
        progress = progress_bar.progress_bar(
            itr,
...
def freeze_layers(model, layer_keywords):
