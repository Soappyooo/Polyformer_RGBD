...
models, cfg, task, tokenizer, generator = None, None, None, None, None

def load_model(model_path, tokenizer_path):
    global models, cfg, task, tokenizer, generator
...
def preprocess_depth(depth_array, roi_ratio=0.2, min_clamp_depth=2):
...
def construct_sample(image: Image, text: str, depth_array: np.ndarray = None):
    ...
    if depth_array is not None:
        # depth_tensor = preprocess_depth(depth_array)
        # patch_depth = depth_resize_transform(depth_tensor).unsqueeze(0)
        depth_array = cv2.resize(depth_array, (patch_image_size, patch_image_size), interpolation=cv2.INTER_NEAREST)
        patch_depth = preprocess_depth(depth_array).unsqueeze(0)
    ...
    sample = {
        ...
        "net_input": {
            ...
            "patch_depths": patch_depth if depth_array is not None else None,
...
def overlay_predictions(
    img, mask=None, polygons=None, bbox=None, color_box=(0, 255, 0), color_mask=[255, 102, 102], color_poly=[255, 0, 0], thickness=3, radius=6
):
    overlayed = img.copy()
    if bbox is not None:
...
def visual_grounding(image, text, depth_array=None):
    # Construct input sample & preprocess for GPU if cuda available
    sample = construct_sample(image, text.lower(), depth_array)
    ...
    with torch.no_grad():
        ...
        encoder_out = model.encoder(
            ...
            patch_depths=sample["net_input"]["patch_depths"],
