...
num_bins=64
log_dir=./mixed/logs
save_dir=./mixed/checkpoints
mkdir -p $log_dir $save_dir

bpe_dir=../../utils/BPE
user_dir=../../polyformer_module

data_dir=../../datasets/mixed
data=${data_dir}/train.tsv,${data_dir}/val.tsv
selected_cols=0
restore_file=../../checkpoints/checkpoint_refcocog.pt
...
for max_epoch in 15; do
  ...
  for lr in 5e-5; do
    ...
    for patch_image_size in 512; do
      ...
      CUDA_VISIBLE_DEVICES=1,4 python3 -m torch.distributed.launch --nproc_per_node=2 --master_port=${MASTER_PORT} ../../train.py \
          $data \
