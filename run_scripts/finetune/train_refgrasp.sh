...
cls_weight=0.005
num_bins=64
log_dir=./refgrasp_rgb/logs
save_dir=./refgrasp_rgb/checkpoints
mkdir -p $log_dir $save_dir

bpe_dir=../../utils/BPE
user_dir=../../polyformer_module

data_dir=../../datasets/refgrasp
data=${data_dir}/train/train.tsv,${data_dir}/val/val.tsv
selected_cols=0,1,2,3,4,5
restore_file=../../checkpoints/checkpoint_mixed.pt


task=refcoco
# arch=polyformer_rgbd_b
arch=polyformer_b
...
for max_epoch in 15; do
  ...
  for lr in 5e-5; do
    ...
    for patch_image_size in 512; do
      ...
      log_file=${log_dir}/${max_epoch}"_"${lr}"_"${patch_image_size}_${current_time}".log"
      ...
      CUDA_VISIBLE_DEVICES=1,2 python3 -m torch.distributed.launch --nproc_per_node=2 --master_port=${MASTER_PORT} ../../train.py \
          $data \
          --selected-cols=${selected_cols} --patience 5 --valid-subset val\
          --bpe-dir=${bpe_dir} \
          --user-dir=${user_dir} \
          ...
          --save-interval-updates=0 --validate-interval-updates=0 \
          --eval-acc \
