...
###
ReferringDataset = RefgraspDataset
task = "res"
###
...
class RefcocoConfig(BaseConfig):
    ...
    eval_args: Optional[str] = field(
        default="{}",
        metadata={"help": 'generation args, e.g., \'{"beam": 4, "lenpen": 0.6}\', as JSON string'},
    )
    ...
    max_image_size: int = field(default=512, metadata={"help": "max image size for normalization"})
...
class RefcocoTask(BaseTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = ReferringDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
    ...
    def _inference(self, sample, model):
        ...
        hyps = self.inference_step_valid(model, sample)
    ...
    def inference_step_valid(self, model, sample):
        with torch.no_grad():
            if isinstance(model, list):
                model = model[0]
            total_len = 2
